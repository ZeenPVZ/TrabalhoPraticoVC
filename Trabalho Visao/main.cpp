...
int main(void) {
    ...
    // Alocar imagens IVC uma vez fora do loop (evita malloc/free por frame)
    ...
    while (key != 'q') {
        ...
        // ============================================================
        // 1. CONVERTER FRAME OpenCV (BGR) PARA IVC (RGB)
        // Usa cvtColor + memcpy em vez de loop pixel a pixel (muito mais rapido)
        // ============================================================
        cv::cvtColor(frame, frame_rgb, cv::COLOR_BGR2RGB);
        memcpy(src_rgb->data, frame_rgb.data, video.width * video.height * 3);

        // ============================================================
        // 2. RGB -> HSV -> SEGMENTACAO (cor laranja)
        // Laranjas: H=[15,30] S=[50,100] V=[40,100]
        // ============================================================
        ...
        // ============================================================
        vc_binary_open(bin, morph, 3);
        vc_binary_close(morph, bin, 7);
            ...
            if (blobs != NULL) {
                ...
                for (int i = 0; i < nlabels; i++) {
                    ...
                    float maior = blobs[i].width > blobs[i].height
                        ? (float)blobs[i].width : (float)blobs[i].height;
        ...
        // ============================================================
        cv::rectangle(frame, cv::Point(0, 0), cv::Point(430, 110),
            cv::Scalar(0, 0, 0), -1);
    ...
    // Libertar memoria IVC no fim (fora do loop)
